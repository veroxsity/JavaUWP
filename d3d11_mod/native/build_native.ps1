...
if (-not $OutputDir) {
    $OutputDir = Join-Path (Get-ConfigPath "BuildDir") "d3d11_backend"
}
$OutputDir = (New-Item -ItemType Directory -Force -Path $OutputDir).FullName
$dllPath = Join-Path $OutputDir "bandit_d3d11_backend.dll"
$objPath = Join-Path $OutputDir "bandit_d3d11_backend.obj"
$libPath = Join-Path $OutputDir "bandit_d3d11_backend.lib"
...
Write-Host "Building bandit_d3d11_backend.dll..."
& $tools.ClExe bandit_d3d11_backend.cpp /LD /EHsc /O2 /D_UNICODE /DUNICODE /D_WIN32_WINNT=0x0A00 /Fo"$objPath" `
    /DWINAPI_FAMILY=WINAPI_FAMILY_APP `
    /link /OUT:"$dllPath" /IMPLIB:"$libPath" /MACHINE:X64 `
    kernel32.lib runtimeobject.lib windowsapp.lib ole32.lib d3d11.lib dxgi.lib d3dcompiler.lib
if ($LASTEXITCODE -ne 0) {
    Pop-Location
    throw "bandit_d3d11_backend build FAILED"
}
Pop-Location

Write-Host "bandit_d3d11_backend.dll built OK -> $dllPath"
