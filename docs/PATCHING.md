...
- `config\forge-install-profile.json` in the repo.
...
- Native mouse input from CoreWindow and MouseDevice, surfaced through GLFW cursor and mouse button callbacks.
...
`26.2` is the exception to step 6. It ships unobfuscated, so Fabric has no intermediary for it and no remapped jar is ever written, and its compatibility mixins target Mojang names rather than intermediary ones. See the Minecraft 26.2 section in [BUILDING.md](BUILDING.md).
