...
public final class BanditControllerCompat {
    ...
    static boolean creativeSearchKeyboardAllowed() {
        return true;
    }
    ...
    public static String radialKeyLabel(String keyId) {
        ...
        class_304 key = keyMappingById(keyId);
        return key == null
            ? "Missing: " + keyId
    ...
    private static class_304[] allKeyMappings() {
        class_310 client = class_310.method_1551();
        return client == null || client.field_1690 == null ? null : client.field_1690.field_1839;
    }

    // 1.21.4 gained KeyMapping.get(id), everything older has to walk the options array
    private static class_304 keyMappingById(String keyId) {
        if (keyId == null || keyId.isEmpty()) {
            return null;
        }
        class_304[] keys = allKeyMappings();
        ...
        for (class_304 key : keys) {
            if (key != null && keyId.equals(key.method_1431())) {
                return key;
            }
        }
        return null;
    ...
    private static void activateRadialSlot(int slot) {
        ...
        class_304 key = keyMappingById(BanditControllerSettings.get().radialSlot(slot));
        if (key == null) {
            return;
        }
    ...
    // chat, command and screenshot only fire from Keyboard.onKey, so setDown alone will not run them
    ...
    private static void resetInactivityTimer(class_310 client) {
        ...
        // controller input goes through KeyMapping.setDown so the 1.21.2 afk fps limiter never sees us
